...
void SplitTree::computeNonEdgeForces(int point_index, float theta, float* neg_f, float* sum_Q)
{
	...
	#ifdef USE_TBB
	tbb::task_group g;
	tbb::combinable<std::vector<float>> combinable_sum(std::vector<float>(QT_NO_DIMS + 1, 0));
        bool doWork = tree_size > 2000;
		...
		float *neg_f_local = &combinable_sum.local()[0];
		float *sum_Q_local = &combinable_sum.local()[dims];
	...
	if (doWork) g.wait();
	combinable_sum.combine_each([&](const std::vector<float> &v) {
		for (int d = 0; d < QT_NO_DIMS; d++)
			neg_f[d] += v[d];
		*sum_Q += v[QT_NO_DIMS];
	});
